...
#define SYS_CLOCK_HIGH 100000 //Время в отчетах для приема байта по UART
...
volatile uint32_t RsSpeed = 9600;     //Скорость работы интерфейса RS
...
//Обьявление для  АЦП
ADC_HandleTypeDef hadc; 		//Структуры для АЦП
...
void Init_Uart_1(void) //volatile uint32_t RsSpeed) //, volatile uint8_t Parity, volatile uint8_t Stopbit)//Функция инициализации UART 1
{
        // Установка параметров порта UART1
        UART_Init(UART_1, OSC_SYSTEM_VALUE/9600, UART_CONTROL1_TE_M | UART_CONTROL1_RE_M |
...
int main(void)
{
	...
	while (1) {
		...
		//Обработка значений АЦП 
       		ADC_SEL_CHANNEL(hadc.Instance, ADC_CHANNEL2);
        	HAL_ADC_SINGLE(hadc.Instance); // Первое измерение для переключение на канал 2.
        	HAL_ADC_WaitValid(&hadc);
        	for (uint32_t j = 0; j < 3; j++)
        	{
            		HAL_ADC_SINGLE_AND_SET_CH(hadc.Instance, (ADC_CHANNEL2+j));
            		holding_data[8+(j<<2)] = HAL_ADC_WaitAndGetValue(&hadc);
		}
